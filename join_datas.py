...
def join_datasets():
    ...
    for filename in club_urls():
        print(filename)
        df_imported = pd.read_csv('database/'+filename+'.csv',sep='\t',)
        df_all = pd.concat([df_imported,df_all],axis=0) 
    df_all.to_csv('database/all.csv',sep='\t', encoding='utf-8', index=False,)
...
def split_dataset():
    ...
    for filename in club_urls():
        ...
        df_country.to_csv('database/'+filename+'.csv',sep='\t', encoding='utf-8', index=False,)
