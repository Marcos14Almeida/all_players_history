 ...
 for x in variable:
  ...
  try:
   if('players' in x or 'nofoto_jugador' in x):
      name = x.split("alt=")[1].split("loading")[0].replace('"','').rstrip().lstrip()
          
      try:
          player_image = "https://cdn.resfu.com/img_data/players/medium/" + x.split("medium/")[1].split("size")[0]
      except:
          player_image = "https://cdn.resfu.com/media/img/nofoto_jugador.png?size=50x&amp;lossy=1"
  
      if(name == name_list_official[soma]): # confere se o nome está na lista de nomes, porque se não o nome é de outro quadro/tabela
          names.append(name)   
  ...
  for i in range (0,len(infos_list)):
      if(i%4==0):
          ...
          if(len(infos_list[i])==0):
              idade.append(30)
          elif(int(infos_list[i])-ajuste_anos<17):
              idade.append(random.randint(20, 35))
          else:    
              idade.append(int(infos_list[i])-ajuste_anos) #idade do jogador na época
      if(i%4==1):
...
has_database = False
try:
 df_imported = pd.read_csv("jogadores.csv",sep='\t',)
 has_database = True
except:
 df_imported = pd.DataFrame([{'year':1900,'club':exception_clubname,'position':'GOL','name':'a','country':'br',
                             'matchs':0,'goals':0,'assists':0,'age':30,'ovr':0,
...
for club in club_url:
    if(df_imported['club'].str.contains(club).any() == False):
      ...
      for year in range (initial_year,2023): 
        ...
        try:
            df = beSoccer(club,year,"https://pt.besoccer.com/time/elenco/"+club+"/"+str(year))   
            print(df.name)
            if df2.empty:
                 df2 = df
            else:     
      ...
      if(has_database == False):
                 df_imported = df_imported.drop(df_imported[df_imported.club== exception_clubname].index)
                 has_database = True       
